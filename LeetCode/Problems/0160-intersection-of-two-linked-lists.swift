...
private class Solution {
    func getIntersectionNode(_ headA: ListNode?, _ headB: ListNode?) -> ListNode? {
        ...
        let diff = abs(firstCount - secondCount)

        var longHead = firstCount > secondCount ? headA : headB
        var shortHead = firstCount > secondCount ? headB : headA

        for _ in 0 ..< diff {
            longHead = longHead?.next
        }

        while longHead != nil && shortHead != nil {
...
class TestSolution0160: XCTestCase {
    func test1() {
